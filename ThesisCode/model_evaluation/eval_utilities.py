...
#Revised sharperatio to better handling the desired hitrate

def MLSharpeRatioHitrate(df, alpha=1.0):
    won_auctions_df = df[df["MLMarginWins"] > 0]
    L = np.mean(won_auctions_df["MLProfit"])
    squared_diffs = (won_auctions_df["MLProfit"] - L) ** 2
    total_squared_diffs = np.sum(squared_diffs)
    sr = (alpha*L) / np.sqrt(total_squared_diffs / len(won_auctions_df))
    return round(sr * 1000, 2)

def DealerSharpeRatioHitrate(df, alpha=1.0):
    won_auctions_df = df[df["Result"] == "Won"]
    L = np.mean(won_auctions_df["DealerProfit"])
    squared_diffs = (won_auctions_df["DealerProfit"] - L) ** 2
    total_squared_diffs = np.sum(squared_diffs)
    sr = (alpha*L) / np.sqrt(total_squared_diffs / len(won_auctions_df))
    return round(sr * 1000, 2)
