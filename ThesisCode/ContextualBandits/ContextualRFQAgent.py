...
class ContextualRFQAgent():
        ...
        if algorithm == 'action_classifier':
            ...
            def get_best_action_for_each_trade(df, model, possible_actions):
                ...
                for _, row in df.iterrows():
                    context_data = pd.DataFrame([row] * len(possible_actions))  
                    context_data['Action'] = possible_actions  
    ...
    def train(self, dataset: pd.DataFrame, algorithm='action_classifier'):
        ...
        df['classifier_label'] = df['Action'].map(spread_to_label)  
        
        if algorithm == 'action_classifier':
            xgb_model = xgb.XGBClassifier(
                objective="multi:softmax",          
                num_class=len(unique_spreads),      
                max_depth=6,                
                eta=0.3,                    
                n_estimators=100,           
                random_state=42
            ...
            context_features = pd.DataFrame(df['State'].tolist())  
            actions = df['Action']  
            rewards = df['Reward']  
