...
def QL(D, gamma, num_states, num_actions):
    ...
    for state in range(num_states):
        if np.all(Q[state, :] == 0):  # Check if the state row in Q-table is all zeros
            optimal_policy.append(most_frequent_action)  # Use the most frequent action as a fallback
        else:
            optimal_policy.append(np.argmax(Q[state, :]))  # Use the action with the highest Q-value

    #If all states were visited infinetly often
    #optimal_policy = np.argmax(Q, axis=1)
...
def FQI(D, gamma, K, num_actions):
    states = np.vstack(D['State'])
    actions = D['Action'].values
    rewards = D['Reward'].values
    next_states = np.vstack(D['Next_State'])

    # Initialize Extra Trees regressor
    n_estimators = 500
    q_regressor = ExtraTreesRegressor(n_estimators=n_estimators)
    ...
    for _ in range(K):
        # Shuffle the dataset
        states, actions, rewards, next_states = shuffle(states, actions, rewards, next_states)
        ...
        for start in range(0, len(states), batch_size):
            ...
            q_values_next = q_regressor.predict(np.hstack((batch_next_states, np.arange(num_actions).reshape(-1, 1))))
            targets = batch_rewards + gamma * np.max(q_values_next, axis=1)
            
            # Fit the Q-function
            q_regressor.fit(np.hstack((batch_states, batch_actions.reshape(-1, 1))), targets)
        
        # Update policy (greedy with respect to Q-function)
        q_values = q_regressor.predict(np.hstack((states, np.arange(num_actions).reshape(-1, 1))))
        optimal_policy = np.argmax(q_values, axis=1)
        
        # Check for convergence (example condition)
        if np.linalg.norm(q_values - targets) < 1e-3:
            break
