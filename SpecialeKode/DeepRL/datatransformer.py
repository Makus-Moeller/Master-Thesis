...
class DataTransformerDeep():
    ...
    def add_hitrate_delta(self, df, window_size):
        ...
        # Transform 'result' column from categorical to numerical
        df['accepted'] = df['Result'].map({'Won': 1, 'Lost': 0})

        # Compute rolling hitrate (h_t) with a default value for the first 10 rows
        df['h_t'] = df['accepted'].rolling(window=window_size, min_periods=1).mean()
        df.loc[:9, 'h_t'] = 0.3  # Default value for the first 10 rows

        # Compute rolling average of market thickness with a default value
        ...
        # Calculate h_desired_t = 1 / (rolling_avg_thickness + 1) with a default value
        df['h_desired_t'] = 1 / (df['rolling_avg_thickness'] + 1)
        df.loc[:9, 'h_desired_t'] = 0.3  # Default value for the first 10 rows
