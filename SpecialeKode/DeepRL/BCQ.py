...
class QNetwork(nn.Module):
    ...
    def forward(self, state, action):
        if action.dim() == 1:  # Only if it's 1D
            action = action.unsqueeze(1)  # Convert to [32, 1]
        
        sa = torch.cat([state, action], dim=1) #sa  = state action pair
...
class PerturbationNetwork(nn.Module):
    ...
    def forward(self, state, action):
        ...
        return 0.1 * self.network(sa)  # Scale the perturbation
    ...
    def train(self, experience_data, train_iterations=1000, batch_size=128, lr=0.001, gamma=0.99, seed=42):
        ...
        for iteration in range(train_iterations):
            ...
            with torch.no_grad():
                ...
                episode_ends = (next_states == -1).all(axis=1)  # Detect terminal states
                next_q_values[episode_ends] = 0  # No future reward for terminal states
            ...
            # Calculate a loss for the perturbation network
            # Here, we'll define a simple loss based on the difference between the perturbed actions and the original actions
            perturb_loss = loss_fn(perturbed_actions.squeeze(1), actions_tensor)  # This is just one possible loss definition
