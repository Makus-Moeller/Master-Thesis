...
class CQLSegmented(CQL):
    ...
    def sample_inventory_distribution(self, segment, batch_size):
        ...
        inventories = np.array([s[0] for s,_,_,_,_ in segment])  # Assuming inventory is the first feature in state
    ...
    def calculate_rewards(self, actions, mu_actions, reward_terms, reward_function, beta=1.0, dist_fn='abs', w1=1.0, w2=1.0, w3=0.5):
        ...
        max_dist = dist.max().item() if dist.max().item() > 0 else 1  # Avoid division by zero
    ...
    def mean_field_policy_random(self, contextual_states_tensor, tau=1.0):
        ...
        with torch.no_grad():
            ...
            # Debugging: Check for NaN or Inf in Qs
            ...
            # Clamp Qs to a reasonable range to avoid extreme values
        ...
        for iteration in range(train_iterations):
            ...
            for start in range(0, len(experience_data) - self.segment_size, self.segment_size - self.overlap):
                ...
                for j in range(policy_iterations):
                    ...
                    mean_field_sample_tensor = torch.tensor(mean_field_sample, dtype=torch.float32).unsqueeze(1)  # Shape (batch_size, 1)
                    ...
                    mu_actions = self.mean_field_policy_random(meanfield_states_tensor, tau=3) #Return tensor
